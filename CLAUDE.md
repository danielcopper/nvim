...
- **`lua/plugins/`**: Plugin configurations (one file per plugin, flat structure)
  ...
  - `lsp.lua`: LSP diagnostics, capabilities, server enablement, keymaps
- **`lsp/`**: Per-server LSP configurations (native 0.11 auto-discovery, one file per server)
...
- `lsp/*.lua`: Per-server configurations (native Neovim auto-discovery — no nvim-lspconfig)
- `lua/config/lsp.lua`: Diagnostic config, global capabilities via `vim.lsp.config('*')`, `vim.lsp.enable()` server list, LspAttach keymaps
